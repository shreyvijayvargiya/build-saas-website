	...
	return (
		<div className="min-h-screen bg-zinc-50 flex flex-col">
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							...
							<motion.button
								...
								className="md:hidden block flex items-center gap-1.5 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-100 rounded-xl transition-colors"
							>
			...
			<main className="flex-1 w-full flex justify-center h-[calc(100vh-60px)] relative">
				...
				<AnimatePresence>
					<div
						className={`fixed lg:static inset-y-0 left-0 w-64 bg-white border-r border-zinc-200 flex flex-col z-50 lg:z-auto ${
							isSidebarOpen
								? "translate-x-0"
					...
					>
						<div className="p-3 border-b border-zinc-200">
							...
							<motion.button
								...
								className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl text-xs font-medium transition-colors mb-2"
							>
									...
									{filteredEmails.map((email) => (
										<motion.div
											...
											className={`p-2 rounded-xl cursor-pointer transition-colors mb-1 ${
												id === email.id
													? "bg-zinc-100 hover:bg-zinc-900 text-black  hover:text-white"
										...
										>
											...
											<div
												className={`text-xs ${
													id === email.id ? "text-zinc-300" : "text-zinc-500"
												}`}
											>
				...
				<AnimatePresence>
					<div
						className={`fixed lg:static inset-y-0 right-0 w-80 bg-white border-l border-zinc-200 p-4 overflow-y-auto z-50 lg:z-auto ${
							isSettingsOpen
								? "translate-x-0"
								: "translate-x-full lg:translate-x-0"
						} transition-transform duration-300 ease-in-out`}
